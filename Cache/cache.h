...
class cache{
    public:
        ...
        cache(cache* nextMem, int assoc, int bksize, int rows); // this is not a copy constructor, it takes in another cache as the next in line
        

        // requests an address
        // if it is not found in this cache, it will check next_cache or ram depending on which was assigned
    ...
    private:
        ...
        // each individual index of the outer vector will refer to a specific cache line in each table 
        // (history[0] refers to line 0 in every table)
        // the inner vector will store in order, when that line was used
        // 0 will be most recently used, while N-1 is the least recently used
